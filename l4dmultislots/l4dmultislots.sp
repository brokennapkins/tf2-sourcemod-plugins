/************************************************
 * Plugin name:		[L4D(2)] MultiSlots OSF
 * Plugin author:	Broken Napkins
 *
 * Based upon:
 * - (L4D) Zombie Havoc by Bigbuck
 * - (L4D2) Bebop by frool
 * - [L4D(2)] MultiSlots by SwiftReal
 *
 * Updated to compile as of 2017-01-17 using
...
public evtPlayerLeftStart(Handle:event, const String:name[], bool:dontBroadcast) {
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (client && IsClientConnected(client) && IsClientInGame(client) && (GetClientTeam(client)==TEAM_SURVIVORS)) {
		gbPlayedAsSurvivorBefore[client] = true;
	}
}
...
public evtPlayerTeam(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (client) {
		if (!IsClientConnected(client)) {
			return;
		} else if (!IsClientInGame(client) || IsFakeClient(client) || !IsAlive(client)) {
			return;
...
public evtPlayerReplacedBot(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (!client) {
		return;
	} else if (GetClientTeam(client)!=TEAM_SURVIVORS || IsFakeClient(client)) {
		return;
	} else if (!gbPlayedAsSurvivorBefore[client]) {
		//ForceClientCvars(client)
		gbPlayedAsSurvivorBefore[client] = true;
		giIdleTicks[client] = 0;
...
public Action:Timer_SpecCheck(Handle:timer) {
	...
	for (new i = 1; i <= MaxClients; i++) {
		if (IsClientConnected(i)
				&& IsClientInGame(i)
				&& (GetClientTeam(i) == TEAM_SPECTATORS)
				&& !IsFakeClient(i)
		) {
			if (!IsClientIdle(i)) {
				new String:PlayerName[100];
...
stock void FindLocationStart() {
	...
	if (StrContains(gMapName, "m1_", false) != -1) {
		...
		while ((ent = FindEntityByClassname(ent, "prop_door_rotating_checkpoint")) != -1) {
			if (IsValidEntity(ent) && GetEntProp(ent, Prop_Send, "m_bLocked") == 1) {
				GetEntPropVector(ent, Prop_Send, "m_vecOrigin", vecLocation);
				vecLocationStart = vecLocation;
				break;
			}
...
stock void TakeOverBot(client, bool:completely) {
	if (!IsClientInGame(client)) {
		return;
...
stock void StripWeapons(client) { // strip all items from client
	new itemIdx;
	for (new x = 0; x <= 3; x++) {
...
stock void GiveWeapon(client) // give client random weapon
{
...
/*
stock void ForceClientCvars(client) {
...
stock int HumanConnected() {
	new humanCount = 0;
	for (new i = 1; i <= MaxClients; i++) {
		if (IsClientConnected(i) && IsClientInGame(bot) && !IsFakeClient(i)) {
			humanCount++;
		}
	}
	return humanCount;
